...
object ContentBasedItemToItemSimilarity {
  val nb_genres = 18
  ...
  def itemSimilarities(sparkContextObject:SparkContext) {
   ...
   // val sparkContextObject = new SparkContext(conf)
    ...
    val movieNamesList = sparkContextObject.textFile("data/movies.csv")
      .mapPartitionsWithIndex { (idx, iter) => if (idx == 0) iter.drop(1) else iter }
      .map(line =>
        {
          val record = line.split(",")
          val movieId = record(0).toLong
          val listGenre = record(1).split(":").toList.map(x => x.toInt)
          (movieId,listGenre)
        })

    //cartesion product: square of movie matrix

    val cartesian: RDD[((Long, List[Int]), (Long,List[Int]))] = movieNamesList.cartesian(movieNamesList)

    val product = cartesian.map {
      case (nelem, melem) =>
       /// if (nelem._1 >= melem._1)
          /*(Some*/((nelem._1, melem._1), (scalarProduct(nelem._2, melem._2)))/*)*/
      //  else
       //   None
    }
    //product.saveAsTextFile("product")
    //diagonal extraction
    val diagonal: Map[Long, Double] = product.filter(tuple => tuple._1._1 == tuple._1._2).
      map(tuple => (tuple._1._1, scala.math.sqrt(tuple._2)))
      .collect()
      .toMap

    val bDiagonal = sqlContext.sparkContext.broadcast(diagonal)
    //item to item cosine similiarity matrix according to genre
    val itemToItemMatrix: RDD[((Long, Long), Double)] = product.flatMap(t => {

      val ((m1, m2), score) = t
      val m1Sqrt = bDiagonal.value.apply(m1)
      val m2Sqrt = bDiagonal.value.apply(m2)
      val cosinSimilarity = score / (m1Sqrt * m2Sqrt)
      if (cosinSimilarity > 0) Some((m1, m2), cosinSimilarity) else None
    })
      .cache()

    /* itemToItemMatrix.map(tuple => {
     ( tuple._1._1, tuple._1._2, tuple._2)}).map(_.productIterator.mkString(":"))
   .saveAsTextFile("itemToitemSimi.txt")*/

    itemToItemMatrix.map(tuple=>  s"${tuple._1._1};${tuple._1._2};${tuple._2}").saveAsTextFile("out3/itemtoitem")
