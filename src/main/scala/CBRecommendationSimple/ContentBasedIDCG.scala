...
object ContentBasedIDCG {
  ...
  def IDCG(sparkContextObject:SparkContext, K:Int) {
  ...
  //  RankInPred.saveAsTextFile("out3/rankinPred")
    ...
    val usersIDCSG = joinPredAndtest.groupBy(tuple => tuple._1) //group by user
      .map(t => { //userDCG = arate(0) + somme(1 to K) arate/log_2 (arank)
        val (u, itr) = t

        val order = itr.toList.sortBy(tuple => tuple._5)(Ordering[Double].reverse).take(K) //sort by pradicted rate
        ...
        while (it.hasNext) {
          ...
          indx = indx + 1
          //   }
          lb.append((next._1, next._2, next._3, next._4, next._5, indx))
          prev = next
        }
        // var userIDCG: Double = 0
        var userIDCG = lb(0)._5
        for (i <- 1 to order.length - 1) {
          //userIDCG = userIDCG + ((Math.pow(2, lb(i)._5) - 1) / Math.log(lb(i)._6 + 1))
          userIDCG = userIDCG + (lb(i)._5 / Math.log(lb(i)._6))
        }
        (u, userIDCG)

      })

  usersIDCSG.map(tuple=>  s"${tuple._1};${tuple._2}").saveAsTextFile("out3/usersIDCSG"+K.toString())
  }
  }
